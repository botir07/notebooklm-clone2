...
export const FlashcardSettingsModal: React.FC<Props> = ({ onClose, onGenerate, isGenerating }) => {
      ...
      <div className="bg-white w-full max-w-2xl rounded-[2rem] shadow-2xl flex flex-col overflow-hidden border border-slate-200/50">
        <header className="p-6 border-b border-slate-100 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-lg font-bold text-slate-800 tracking-tight">Kartochka sozlamalari</h2>
          </div>
        ...
        <div className="p-8 space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <div className="space-y-3">
              <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Kartalar soni</label>
              <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-200">
                {[8, 12, 16].map((val) => (
                  <button
                    key={val}
                    onClick={() => setSettings({...settings, cardCount: val as any})}
                    className={`flex-1 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${settings.cardCount === val ? 'bg-white text-indigo-600 shadow-sm border border-slate-100' : 'text-slate-400 hover:text-slate-600'}`}
                  >
                    {val} ta
                  </button>
                ))}
          ...
          <div className="space-y-3">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Murakkablik</label>
            <div className="flex bg-slate-50 p-1 rounded-2xl border border-slate-200">
          ...
          <div className="space-y-3">
            <label className="text-[11px] font-black text-slate-400 uppercase tracking-widest">Fokus mavzusi</label>
            <textarea
              value={settings.description}
              onChange={(e) => setSettings({...settings, description: e.target.value})}
              placeholder="Masalan: 'Faqat asosiy terminlarga e'tibor qarating'"
        ...
        <footer className="p-6 bg-slate-50 border-t border-slate-100 flex justify-end">
          <button
            ...
            className="flex items-center gap-3 px-10 py-4 bg-rose-600 text-white rounded-2xl font-black text-xs uppercase tracking-widest hover:bg-rose-700 transition-all shadow-xl disabled:opacity-50"
          >
